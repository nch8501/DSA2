...
void Application::Display(void)
{
	...
	static uint uCurrentStop = 0;

	//increase percentage
	fPercentage = fTimer / 2;

	//check if guy is at last stop
	if (uCurrentStop == m_stopsList.size() - 1)
	{
		v3CurrentPos = glm::lerp(m_stopsList[uCurrentStop], m_stopsList[0], fPercentage);
	}
	else
	{
		v3CurrentPos = glm::lerp(m_stopsList[uCurrentStop], m_stopsList[uCurrentStop + 1], fPercentage);
	}

	//check if guy has made it to the stop
	if (fPercentage >= 1.0f)
	{
		if (uCurrentStop == m_stopsList.size() - 1)
		{
